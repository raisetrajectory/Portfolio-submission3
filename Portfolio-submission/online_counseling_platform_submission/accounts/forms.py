...
# from cloudinary.forms import CloudinaryFileField #2024年8月追加

# class UserEditForm(forms.ModelForm): #2024年8月追加
#     username = forms.CharField(label='ユーザーネーム')
#     age = forms.IntegerField(label='年齢', min_value=0)
#     email = forms.EmailField(label='メールアドレス')
...
class CounselorEditForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        instance = super().save(commit=False)
        # カウンセラーとしてのステータスを強制的にTrueにする
        instance.is_counselor = True
        if commit:
            instance.save()
        return instance

# class CounselorEditForm(forms.ModelForm):　#2024年8月追加
#     username = forms.CharField(label='カウンセラーネーム')
#     age = forms.IntegerField(label='年齢', min_value=0)
#     email = forms.EmailField(label='メールアドレス')
#     picture = CloudinaryFileField(label='写真', required=False)
