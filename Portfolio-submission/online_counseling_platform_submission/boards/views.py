...
#             else:
#                 create_theme_form = forms.CreateThemeForm()
#             users = Users.objects.filter(counselor__isnull=True)  # Filter users who are not counselors
#             return render(request, 'boards/create_theme.html', context={
#                 'create_theme_form': create_theme_form,
#                 'user_type': user_type,
#                 'users': users
...
def create_theme(request):
    if request.user.is_authenticated:
        if isinstance(request.user, Users):
            ...
            users = Users.objects.filter(username__in=['カウンセラー01', 'カウンセラー02'])  # カウンセラー01とカウンセラー02のみを選択可能にする
            return render(request, 'boards/create_theme.html', context={
                'create_theme_form': create_theme_form,
                'user_type': user_type,
                'users': users
