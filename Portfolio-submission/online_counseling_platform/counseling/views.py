...
# @login_required #2024年6月8日追加 記載内容のバックアップです！
# def chat_view(request, session_id=None, counselor_id=None):
#     session = None
#     if session_id:
#         session = get_object_or_404(CounselingSession, id=session_id)
...
#     elif counselor_id:
#         counselor = get_object_or_404(Counselor, id=counselor_id)
#         session = CounselingSession.objects.get_or_create(user=request.user, counselor=counselor)
...
#             chat_message.session = session
#             chat_message.save()
#             # After saving the message, redirect to the same page to avoid resubmission
...
#     # Get all chat messages related to the session
#     messages = ChatMessage.objects.filter(session=session).order_by('timestamp')
...
def chat_view(request, session_id=None, counselor_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            chat_message.session = session
            chat_message.save()
            form = ChatMessageForm(initial={'session_id': session.id})  # Clear form after saving message
