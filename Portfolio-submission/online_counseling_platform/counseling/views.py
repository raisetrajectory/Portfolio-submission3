...
def get_actual_user(user): #2024年6月10日追加
    if isinstance(user, SimpleLazyObject) and hasattr(user, '_wrapped'):
...
#             chat_message.session = get_object_or_404(CounselingSession, id=form.cleaned_data['session_id'])
#             chat_message.save()
#             # チャット画面にリダイレクトする際に、session_idと一緒にchat_messageのidを渡す
...
# messages = []  # メッセージの初期化　#2024年6月9日追加 この記載内容に戻りましょう！
...
#             form.instance.sender = request.user._wrapped if isinstance(request.user, SimpleLazyObject) else request.user  # senderにrequest.userをセット
#             form.save()
...
#             message = form.cleaned_data['message']
#             form.instance.session_id = session_id
#             form.instance.sender = request.user if not isinstance(request.user, AnonymousUser) else None
#             form.save()
#             return redirect('chat_view', session_id=session_id)  # チャット画面にリダイレクト
#     else:
#         form = ChatMessageForm(initial={'session_id': session_id})
...
#             message = form.cleaned_data['message']
#             form.instance.session_id = session_id
#             form.instance.sender = request.user._wrapped if hasattr(request.user, '_wrapped') else request.user  # senderにrequest.userをセット
#             form.save()
#             return redirect('chat_view', session_id=session_id)  # チャット画面にリダイレクト
#     else:
#         form = ChatMessageForm(initial={'session_id': session_id})
...
def chat_view(request, session_id=None, counselor_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            message = form.cleaned_data['message']
            form.instance.session_id = session_id
            form.instance.sender = get_actual_user(request.user)  # senderにrequest.userをセット
            form.save()
            return redirect('chat_view', session_id=session_id)  # チャット画面にリダイレクト
    else:
        form = ChatMessageForm(initial={'session_id': session_id})
